...
def my_job():
    ...
    for mail in dataset:
        ...
        mail.save()
            ...
            # else:
            #     for client in mail.clients.all():
            #         # result = MailingLog.objects.filter(mail=mail).filter(client=client).aggregate(Max('time'))
            #         # print('result', result)
            #         if today <= mail.next_time_mailing:
            #             mail.mailing_status = 'executing'
            #             if mail.mail_regularity == 'per_day':
            #                 mail.next_time_mailing += timedelta(days=1)
            #             mail.save()
                    # if result['time__max'] is None:
                    #     last_time = datetime(2020, 1, 1, 0, 0)
                    # else:
                    #     last_time = datetime(result['time__max'])